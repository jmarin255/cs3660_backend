...
@router.post("/", response_model=LoginRequest)
...
async def verify(verify_request:VerifyLoginRequest):
    try:
        ...
        return LoginResponse(sucess= True, jwt_token=verify_request.jwt_token)
    except Exception as e:
        raise HTTPException(status_code=401, detial=str(e))    
